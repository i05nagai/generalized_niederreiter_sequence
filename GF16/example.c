...
int main( int ac, char **av ){
  ...
  printf("base = 16, dim = %d, Seq_type = %d, jump = %lu, seed = %lu, num = %lu\n", dim, Seq_type, jump, seed,  num);
  printf("flug_g = %d, flug_l = %d, flug_s = %d\n", flug_g, flug_l, flug_s);
  for(i=0;i<num;i++){
    LDS_GF16( dim, v, Seq_type, jump, seed, flug_g, flug_l, flug_s);
    for(j=0;j<dim;j++)
      printf("%12.8lf",v[j]);
      printf("\n");
  }
  ...
  printf("flug_g = %d, flug_l = %d, flug_s = %d\n", flug_g, flug_l, flug_s);
  for(i=0;i<num;i++){
    LDS_Gauss_GF16( dim, v, Seq_type, jump, seed, flug_g, flug_l, flug_s);
    for(j=0;j<dim;j++)
      printf("%12.8lf",v[j]);
      printf("\n");
      }*/
