      ...
      > operation)
{
  ...
  for (size_t i = 0; i < test_case.size(); i += 3) {
    ...
    if (expect != test_case[i + 2]) {
      return ::testing::AssertionFailure()
        << (i / 3) << " th test_case is different. "
        << test_case[i] << " op " << test_case[i + 1] << ", "
        << " expect: " << expect.value() << ", "
...
inline
::testing::AssertionResult
IsExpectEqualWithIndex(const T1& expect, const T2& actual, const Index& i)
{
  if (expect != actual) {
    return ::testing::AssertionFailure()
      << i << " th test is different. "
      << " expect: " << expect << ", "
      << " actual: " << actual;
  }
  return ::testing::AssertionSuccess();
